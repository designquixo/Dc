...
export default async function handler(req, res) {
  ...
  try {
    ...
    currentReviews = currentReviews.filter(r => r.id !== id && String(r.id) !== String(id));
    ...
    console.error('Error in delete-review handler:', err);
    return res.status(500).json({ success: false, message: err?.message || 'Error deleting review' });
  }
}
